...
OUTPUT_FILE = '/Users/gary/Documents/Supermarket1/Json_data/filtered_powder_drinks.json'

def load_products():
    """
    读取两个 JSON，筛出「沖調飲品」子分类的记录，仅保留 uid / eng 字段
    """
    items = []
    for path in INPUT_FILES:
        ...
        for p in wrapper.get('products', []):
            # ❗️子分类里必须有 “沖調飲品”
            if '沖調飲品' not in p.get('sub_categories', []):
                continue
            uid = p.get('uid')
    ...
    logging.info(f'载入沖調飲品原始记录：{len(items)} 条')
    return items
