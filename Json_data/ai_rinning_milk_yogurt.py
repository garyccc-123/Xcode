...
2. 對 filtered_milk_yogurt.json 依「品牌 → 系列/口味 → 容量」三層聚類。
3. 聚類結果直接寫回 rec["norm"]，並輸出 normalized_milk_yogurt.json，
...
def main() -> None:
    ...
    # 2) 讀待處理檔
    src = os.path.join(base, "filtered_milk_yogurt.json")
    with open(src, encoding="utf-8") as f:
        data = json.load(f)
    if not data:
        raise ValueError("filtered_milk_yogurt.json 為空！")
   ...
   # 3) LLM prompt ----------------------------------------------------------
    ...
    out = os.path.join(base, "normalized_milk_yogurt.json")
