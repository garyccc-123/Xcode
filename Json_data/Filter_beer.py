...
OUTPUT_FILE = '/Users/gary/Documents/Supermarket1/Json_data/filtered_beer.json'

def load_products():
    """
    读取两个 JSON，筛出「啤酒」子分类的记录，仅保留 uid / eng 字段
    """
    items = []
    for path in INPUT_FILES:
        ...
        for p in wrapper.get('products', []):
            # ❗️子分类里必须有 “啤酒”
            if '啤酒' not in p.get('sub_categories', []):
                continue
            uid = p.get('uid')
    ...
    logging.info(f'载入啤酒原始记录：{len(items)} 条')
    return items
