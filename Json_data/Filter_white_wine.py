...
OUTPUT_FILE = '/Users/gary/Documents/Supermarket1/Json_data/filtered_white_wine.json'

def load_products():
    """
    读取两个 JSON，筛出「白酒」子分类的记录，仅保留 uid / eng 字段
    """
    items = []
    for path in INPUT_FILES:
        ...
        for p in wrapper.get('products', []):
            # ❗️子分类里必须有 “白酒”
            if '白酒' not in p.get('sub_categories', []):
                continue
            uid = p.get('uid')
    ...
    logging.info(f'载入白酒原始记录：{len(items)} 条')
    return items
